...

{
	...
	arq = fopen(url,"r"); // funcao que abre o arquivo // "r" read

	if(arq==NULL) //arquivo vazio
		printf("Nao foi possivel abrir o arquivo\n");
	else {
		while((fgets(nome, sizeof(nome),arq)) !=NULL ){ //le cada nome de cidade do arquivo
			int h = hash(nome, m); //calcula o hash
			cidades[h]++; //add uma cidade naquela posição de hash
			i++;
		}
	}
...
int hash(char v[], int m) {
   ...
   unsigned int h = v[0];
   for (i = 1; v[i] != '\0'; i++)
      h = (h * 251 + v[i]) % m; //251 pq é o numero primo mais prox da quantidade final da tabela ascii 
    // m -> primo qualqer
   return h;
}
